...
<div class="page-content stagger-children">
  ...
  styles: [`
    ...
    .device-screen { width: 140px; height: 100px; background: linear-gradient(135deg, #1E293B, #0F172A); border-radius: 12px 12px 4px 4px; border: 3px solid #334155; display: flex; align-items: center; justify-content: center; }
...
export class GestionBoitierComposant implements OnInit {
  ...
  synchroniserBoitier(): void {
    ...
    setTimeout(() => {
      this.boitier.update(b => b ? { ...b, derniereSync: new Date() } : null);
      this.synchroEnCours.set(false);
      this.notifService.succes('Synchronisation réussie', 'Toutes les données d\'apprentissage ont été synchronisées.');
    }, 1500);
  }
  ...
  connecterWifi(): void {
    ...
    this.boitier.update(b => b ? { ...b, wifiConnecte: true, ssid: this.nouveauSsid } : null);

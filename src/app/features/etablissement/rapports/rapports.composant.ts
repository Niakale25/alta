...
<div class="page-content stagger-children">
  ...
  `,
  styles: [`
    .rapport-generator { background: var(--color-bg-surface); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 24px; margin-bottom: 24px; box-shadow: var(--shadow-sm); }
    .rapport-generator__options { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px; }
    .rapport-type-btn { display: flex; align-items: center; gap: 12px; padding: 14px; border: 1.5px solid var(--color-border); border-radius: var(--radius-md); background: var(--color-bg-app); cursor: pointer; text-align: left; transition: all var(--transition-fast); }
    .rapport-type-btn:hover { border-color: var(--color-primaire); background: var(--color-primaire-light); }
    .rapport-type-btn--active { border-color: var(--color-primaire) !important; background: var(--color-primaire-light) !important; box-shadow: var(--shadow-glow-primaire); }
    .rapport-type-btn__icon { width: 40px; height: 40px; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  `],
})

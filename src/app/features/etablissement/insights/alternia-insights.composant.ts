...
<div class="page-content stagger-children">
  ...
  @if (chargement()) {
    ...
    <!-- Section Top KPI: Indice d'engagement & Activité Temps Réel -->
    <div class="insights-hero-grid">
      <!-- Card Indice d'Engagement Global -->
      <div class="card engagement-hero-card">
        <div class="card__header" style="border:none;padding-bottom:0;">
          <h2 class="card__title">Indice d'Engagement Global</h2>
          <span class="badge badge-success">{{ engagement()?.niveau }}</span>
        </div>
        <div class="card__body" style="display:flex;align-items:center;gap:24px;flex-wrap:wrap;">
          <!-- Circular Gauge -->
          <div class="gauge-container">
            <svg viewBox="0 0 120 120" class="gauge-svg">
              <circle cx="60" cy="60" r="50" fill="none" stroke="var(--color-bg-surface-2)" stroke-width="10"/>
              <circle cx="60" cy="60" r="50" fill="none" stroke="var(--color-secondaire)" stroke-width="10"
                [style.stroke-dasharray]="(engagement()!.scoreGlobal / 100) * 314 + ' 314'"
                stroke-linecap="round"
                transform="rotate(-90 60 60)"
                style="transition: stroke-dasharray 1.2s cubic-bezier(0.34, 1.56, 0.64, 1);"/>
            </svg>
            <div class="gauge-text">
              <span class="gauge-val">{{ engagement()?.scoreGlobal }}</span>
              <span class="gauge-max">/100</span>
            </div>
          </div>

          <div style="flex:1;min-width:200px;">
            <div class="text-xs text-secondary" style="margin-bottom:8px;">Calculé d'après 4 piliers d'apprentissage :</div>
            <div class="composantes-grid">
              <div class="comp-item">
                <span class="comp-label">Temps d'utilisation</span>
                <div class="progress-bar"><div class="progress-bar__fill primary" [style.width.%]="engagement()?.composantes?.tempsUtilisation"></div></div>
                <span class="comp-val">{{ engagement()?.composantes?.tempsUtilisation }}%</span>
              </div>
              <div class="comp-item">
                <span class="comp-label">Fréquence de connexion</span>
                <div class="progress-bar"><div class="progress-bar__fill success" [style.width.%]="engagement()?.composantes?.frequenceConnexion"></div></div>
                <span class="comp-val">{{ engagement()?.composantes?.frequenceConnexion }}%</span>
              </div>
              <div class="comp-item">
                <span class="comp-label">Diversité des matières</span>
                <div class="progress-bar"><div class="progress-bar__fill innovation" [style.width.%]="engagement()?.composantes?.diversiteMatieres"></div></div>
                <span class="comp-val">{{ engagement()?.composantes?.diversiteMatieres }}%</span>
              </div>
              <div class="comp-item">
                <span class="comp-label">Volume de questions IA</span>
                <div class="progress-bar"><div class="progress-bar__fill" style="background:var(--color-secondaire)" [style.width.%]="engagement()?.composantes?.volumeQuestions"></div></div>
                <span class="comp-val">{{ engagement()?.composantes?.volumeQuestions }}%</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Live Activity Panel -->
      <div class="card live-panel-card">
        <div class="card__header">
          <h2 class="card__title">
            Activité Temps Réel
            <span class="status-dot online animate-pulse" style="margin-left:6px;"></span>
          </h2>
          <span class="text-xs text-secondary">Bamako · En direct</span>
        </div>
        <div class="card__body" style="display:flex;flex-direction:column;gap:16px;">
          <div class="live-metric">
            <div class="icon-box icon-box-innovation"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><line x1="12" y1="20" x2="12.01" y2="20"/></svg></div>
            <div style="flex:1;">
              <div class="fw-bold text-lg">342 / 350</div>
              <div class="text-xs text-secondary">Boîtiers connectés actuellement</div>
            </div>
            <span class="badge badge-success">97.7% actifs</span>
          </div>

          <div class="live-metric">
            <div class="icon-box icon-box-primary"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div>
            <div style="flex:1;">
              <div class="fw-bold text-lg">1 420 questions</div>
              <div class="text-xs text-secondary">Questions IA posées aujourd'hui</div>
            </div>
            <span class="text-xs text-success fw-bold">↑ +24% vs hier</span>
          </div>

          <div class="live-metric">
            <div class="icon-box icon-box-secondaire"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></div>
            <div style="flex:1;">
              <div class="fw-bold text-base">Mathématiques & Physique</div>
              <div class="text-xs text-secondary">Matières prédominantes ce jour</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Section Tendances d'apprentissage -->
    <div class="card" style="margin-top:24px;">
      <div class="card__header">
        <h2 class="card__title">Tendances d'apprentissage par Matière</h2>
        <span class="badge badge-primary">Analyse 7 jours</span>
      </div>
      <div class="card__body">
        <div class="tendances-grid">
          @for (tendance of tendances(); track tendance.matiere) {
            <div class="tendance-card" [style.border-top-color]="MatiereCouleurs[tendance.matiere]">
              <div class="tendance-card__header">
                <div>
                  <div class="fw-bold text-base">{{ MatiereLabels[tendance.matiere] }}</div>
                  <div class="text-xs text-secondary">{{ tendance.nombreConsultations.toLocaleString('fr-FR') }} consultations</div>
                </div>
                <span class="badge" [class.badge-success]="tendance.statut === 'hausse'" [class.badge-danger]="tendance.statut === 'baisse'" [class.badge-primary]="tendance.statut === 'stable'">
                  {{ tendance.variationHebdoPct > 0 ? '+' : '' }}{{ tendance.variationHebdoPct }}%
                </span>
              </div>

              <!-- Sparkline SVG -->
              <div class="sparkline-container">
                <svg viewBox="0 0 160 40" preserveAspectRatio="none" style="width:100%;height:40px;">
                  <path [attr.d]="generateSparklinePath(tendance.historique7jours)" fill="none" [attr.stroke]="MatiereCouleurs[tendance.matiere]" stroke-width="2.5" stroke-linecap="round"/>
                </svg>
              </div>

              <div class="tendance-card__footer">
                <span class="text-xs text-secondary">Évol. mensuelle : <strong>+{{ tendance.variationMensuellePct }}%</strong></span>
              </div>
            </div>
          }
        </div>
  ...
  styles: [`
    ...
    .engagement-hero-card { background: linear-gradient(135deg, var(--color-bg-surface), var(--color-bg-surface-2)); border: 1px solid var(--color-border); }
